...
- To assess a classifier's performance as fair we can use the p-rule metric. This is defined as follows.
    - $\min \left(\frac{P(C(x|\theta_{C}^\ast) > \tau ? 1 : 0 | z = 1)}{P(C(x|\theta_{C}^\ast) > \tau ? 1 : 0 | z = 0)}, \frac{P(C(x|\theta_{C}^\ast) > \tau ? 1 : 0 | z = 0)}{P(C(x|\theta_{C}^\ast) > \tau ? 1 : 0 | z = 1)} \right) \geq \frac{p} {100}$
